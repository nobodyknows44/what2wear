...
export default function WeatherDashboard({ showRecommendations, setShowRecommendations }: WeatherDashboardProps) {
  ...
  return (
    <section className="w-full min-h-screen flex items-center justify-center">
      <Card 
        className={`
          w-[90%] max-w-3xl backdrop-blur-sm bg-white/90 
          border border-white/50 shadow-lg rounded-3xl 
          transition-all duration-1000 ease-out
          transform ${isVisible 
            ? 'translate-y-0 opacity-100' 
            : 'translate-y-10 opacity-0'
          }
          ${!showRecommendations ? 'max-w-md' : ''}
        `}
      >
        {!showRecommendations ? (
          <CardContent className="my-8 px-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2" ref={countryRef}>
                ...
                {showCountryDropdown && (
                  <div className="mt-1 max-h-40 overflow-auto absolute z-10 w-[calc(90%-2rem)] md:w-[400px] left-1/2 -translate-x-1/2 bg-white border border-gray-300 rounded-md shadow-lg">
